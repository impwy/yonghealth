...
## 작업 워크플로우
0. **요청 수신 즉시 `requirement.md` Inbox에 원문을 기록한다** (날짜 포함). 세션이 끊겨도 유실되지 않도록, 구현 착수 전에 먼저 기록한다.
1. `requirement.md`의 Inbox/진행 중 섹션을 읽고 고객 요구사항을 파악한다 (새 세션 시작 시에도 동일)
...
7. 요청이 완료되면 `requirement.md`의 Inbox 항목을 "완료" 섹션으로 이동한다 (삭제 금지)
