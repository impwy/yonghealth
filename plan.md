...
### Bug Fix: 프론트엔드 빌드 리소스 폭증
1. 콜드 빌드에서 `next/font/google` 외부 fetch 의존 여부 확인
2. Next.js 16 기본 Turbopack 빌드와 Webpack 빌드의 산출물/안정성 비교
3. `build` 스크립트를 `next build --webpack`으로 고정
...
3. 스핀마다 후보 목록, 당첨 인덱스, 누적 회전 각도, 배정 결과를 `RoulettePlan`으로 생성
4. 룰렛 휠을 실제 회전 애니메이션으로 렌더링하고 당첨자 위치에서 멈추도록 구현
5. 당첨 멤버를 팀별 티어합, 티어 분포, 인원 수 균형 기준으로 배정
6. 룰렛 진행 중 저장/복사/모드 전환을 잠그고 완료 후 최종 편성 저장 가능하게 처리
