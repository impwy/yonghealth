    ...
    <div className="border border-gray-200 rounded-lg bg-white mb-3">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-400">{exercise.sets.length}세트</span>
        </div>
        <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
          {editing ? (
            <>
              <button onClick={handleUpdate} className="text-sm text-blue-600 hover:underline">저장</button>
              <button onClick={() => { setEditing(false); setName(exercise.name); }} className="text-sm text-gray-400 hover:underline">취소</button>
            </>
          ) : (
            <>
