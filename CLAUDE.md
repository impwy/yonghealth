...
0. 사용자의 발화를 먼저 분류한다.
   - 사용자가 "요구사항", "이건 요구사항", "요구사항:"처럼 명시한 항목만 `requirement.md` Inbox에 기록하고 git에 포함한다.
   - 그 외 일반 요청, 운영 지시, 실행/확인 요청은 `request.local.md`에 기록한다. 이 파일은 `.gitignore`에 포함되어 git에 올리지 않는다.
1. 새 세션 시작 시 `requirement.md`의 Inbox/진행 중 섹션을 읽어 확정된 고객 요구사항을 파악하고, 필요하면 `request.local.md`에서 최근 일반 요청 맥락을 확인한다.
...
7. 명시된 요구사항이 완료되면 `requirement.md`의 Inbox 항목을 "완료" 섹션으로 이동한다 (삭제 금지). 일반 요청 완료 기록은 `request.local.md`에만 남긴다.


## 주요 문서
- `requirement.md`: 사용자가 명시적으로 "요구사항"이라고 부른 고객 요구사항 (git 포함)
- `request.local.md`: 일반 요청, 운영 지시, 확인 요청의 로컬 로그 (git 제외)
